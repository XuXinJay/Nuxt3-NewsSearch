    ...
    >
      ...
      >
        <a
          ...
          :key="item.id"
          :href="item.url"
          class="group text-[black] dark:text-[#F5F5F5] bg-[#F5F5F5] dark:bg-[#1e1e20] rounded-xl border-2 p-2"
          target="_blank"
        >
          <h2 class="sr-only">{{ item.title }}</h2>
          <div
            class="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-lg xl:aspect-h-8 xl:aspect-w-7 bg-[#F5F5F5] dark:bg-[#1e1e20]"
          >
            <img
              :src="item.urlToImage"
              class="h-full w-full object-cover object-center group-hover:scale-110 duration-700 min-h-[200px]"
            />
          </div>
          <h3
...
const { data } = await useFetch(
  `https://newsapi.org/v2/everything?q=Business&sortBy=publishedAt&apiKey=${apiKey}`
);
console.log(data);
